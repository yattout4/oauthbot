  ...
  if (command === "info") {
    ...
    if (commandInfo[authorId]) {
      ...
      const updateInfoMessage = () => {
        const timestamp = new Date(commandInfo[authorId].timestamp);
        const formattedDate = `${timestamp.getDate().toString().padStart(2, '0')}/${(timestamp.getMonth() + 1).toString().padStart(2, '0')}/${timestamp.getFullYear()} à ${timestamp.getHours().toString().padStart(2, '0')}h${timestamp.getMinutes().toString().padStart(2, '0')}`;
        const infoEmbed = new Discord.MessageEmbed()
          ...
          `Ajout de membres en cours :\n\n**${serverName} :**\n `+
            ...
            `Temps approximatif : \`${approximateTime}\``
          );
        ...
        if (originalMessageId) {
          message.channel.messages.fetch(originalMessageId)
            .then(msg => {
              originalMessage = msg;
              originalMessage.edit(infoEmbed)
                .then(() => {
...
if (isJoining) {
  ...
  return;
    ...
    const confirmationEmbed = new Discord.MessageEmbed()
      ...
      .setAuthor('Auths 2 - Join Progress on' + serverName)
      .setDescription(`**Join in progress :** \`${totalJoined}\`/\`${numPeople}\``);
    const joinMessage = await message.channel.send(confirmationEmbed);
    
    const incrementInterval = setInterval(() => {
      if (totalJoined < numPeople) {
        totalJoined++;
        commandInfo[message.author.id].totalJoined = totalJoined;
    
        const updatedConfirmationEmbed = new Discord.MessageEmbed()
          .setColor("#313338")
          .setAuthor('Auths 2 - Join Progress on' + serverName)
          .setThumbnail(message.guild.iconURL())
          .setDescription(`**Join in progress :** \`${totalJoined}\`/\`${numPeople}\``);
